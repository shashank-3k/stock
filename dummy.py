...
if st.button('Predict'):
    ...
    url = 'https://eodhistoricaldata.com/api/sentiments?s={}&from=2000-01-01&to=2023-06-30&api_token=6481b11d245909.63698937'.format(stock)
    print(url)
    response = requests.get(url)
    data = response.json()
    ...
    url = 'https://eodhistoricaldata.com/api/tweets-sentiments?s={}&from=2000-01-01&to=2023-06-30&api_=6481b11d245909.63698937'.format(stock)
    response = requests.get(url)
    data = response.json()
    ticker = list(data.keys())[0]
    ...
    def clean_dataset(df):
        ...
        indices_to_keep = ~df.isin([np.nan, np.inf, -np.inf]).any(axis=1) & ~df.select_dtypes(include='datetime64').any(axis=1)
        cleaned_df = df[indices_to_keep].copy()
        cleaned_df = cleaned_df.astype({col: np.float64 for col in cleaned_df.columns})
        return cleaned_df
